...
async function handleUserPanel(req, res) {
  try {
      const tailors = await Blog.find({ }); // Adjust query based on your role definition
...
async function handleSign_Up(req, res) {
  try {
      ...
      console.log("Record inserted successfully");
      if (role == "ADMIN") {
        const tailors = await Blog.find({});
        return res.render("adminPanel", { tailors }); // Pass tailors data to adminPanel.ejs
      } else if (role == "USER") {
        const tailors = await Blog.find({});
        return res.render("userPanel", { tailors }); // Pass tailors data to userPanel.ejs
      } else {
        return res.render("sign_up.html"); // Pass tailors data to index.ejs
      }
